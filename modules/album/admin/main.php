...
if ($nv_Request->isset_request("action", "post,get")) {
    ...
    if ($id > 0 and $checksess == md5($id . NV_CHECK_SESSION)) {
        $db->query("DELETE FROM `nv4_album` WHERE id=" . $id);
    }
}

if ($nv_Request->isset_request("change_active", "post,get")) {
    $id = $nv_Request->get_int('id', "post, get", 0);
    $sql = "SELECT id, active FROM nv4_album WHERE id = " . $id;
    $result = $db->query($sql);
    if ($row = $result->fetch()) {
        $active = $row['active'] == 1 ? 0 : 1;
        $exe = $db->query("UPDATE `nv4_album` SET active =" . $active . " WHERE id=" . $id);
        if ($exe) {
            die("OK");
        }
    }
...
if ($nv_Request->isset_request("change_weight", "post,get")) {
    ...
    if ($id > 0 and $new_weight > 0) {
        $sql = "SELECT id, weight FROM nv4_album WHERE id !=  " . $id;
        $result = $db->query($sql);
        $weight = 0;
        while ($row = $result->fetch()) {
            ...
            $exe = $db->query("UPDATE `nv4_album` SET weight =" . ($weight) . " WHERE id=" . $row['id']);
        }
        $exe = $db->query("UPDATE `nv4_album` SET weight =" . $new_weight . " WHERE id=" . $id);
        if ($exe) {
            die('OK');
        }
    }
